...
export default function AddLog(props) {
  ...
  const hasFilledData = () => {
    ...
    notFilledIndices.length > 0 && dispatch(showSnack(errString));
    return notFilledIndices.length == 0;
  };
    ...
    if (contactPresenceStatus) {
      ...
      for (let i = 0; i < indicies.length; i++) {
        if (logData[indicies[i]].dataValue.phoneNumber.length != 10) {
          dispatch(
            showSnack(
              "Enter correct phone number, " + logData[indicies[i]].dataName
            )
          );
          setIsLoading(false);
